...
const columns: ColumnDef<any>[] = [
    ...
    { accessorKey: "user.email", header: () => <div>Email</div> },
    {
        ...
        cell: ({ row }) => {
            ...
            return s === "PENDING" ? <Link href={`/dashboard/partner_search?accountsOnly=true&phone=${p}`}>VIEW&rarr;</Link> : <div>{s}</div>;
        },
    },
];
        ...
        <div className="my-2">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-4">
                <Card className="relative overflow-hidden bg-primary/10">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{data.partnerARD.reduce((sum: any, entry: any) => sum + entry._count.status, 0)}</div>
                ...
                <Card className="relative overflow-hidden bg-yellow-500/10">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{data.partnerARD.find((entry: any) => entry.status === "PENDING")?._count.status || 0}</div>
                ...
                <Card className="relative overflow-hidden bg-green-500/10">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{data.partnerARD.find((entry: any) => entry.status === "ACCEPTED")?._count.status || 0}</div>
                ...
                <Card className="relative overflow-hidden bg-red-500/10">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{data.partnerARD.find((entry: any) => entry.status === "DUPLICATE")?._count.status || 0}</div>
